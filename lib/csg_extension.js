...
CSG.extrudeFromFace = function(face,geometry,depth) {
  var faceVertices = geometry.vertices;
  var vertices = [], polygons = [];
  var normal = new CSG.Vector(face.normal);
  var flipNormal = normal.negated;
  var extrusionVector = normal.clone().times(depth);

  vertices.push(new CSG.Vertex(faceVertices[face.c],flipNormal));
  vertices.push(new CSG.Vertex(faceVertices[face.b],flipNormal));
  vertices.push(new CSG.Vertex(faceVertices[face.a],flipNormal));

  console.log('extrusionVector:', extrusionVector);
  console.log('before vertices[0].pos=', vertices[0].pos);
  var bottom = new CSG.Polygon(vertices);
  polygons.push(bottom);

  // Now make the sides.

  // Finally make the top
  var nextVertices = [];
  nextVertices.push(new CSG.Vertex(vertices[0].pos.plus(extrusionVector), normal));
  nextVertices.push(new CSG.Vertex(vertices[1].pos.plus(extrusionVector), normal));
  nextVertices.push(new CSG.Vertex(vertices[2].pos.plus(extrusionVector), normal));

  console.log('nextVertices:', nextVertices);
  var top = new CSG.Polygon(nextVertices);
  polygons.push(top);
