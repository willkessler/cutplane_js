...
CSG.prototype.saveVertexBackups = function() {
  for (var polygon of this.polygons) {
    for (var vertex of polygon.vertices) {
      vertex.backup = new CSG.Vector(vertex.pos.x,vertex.pos.y,vertex.pos.z);
      vertex.normalBackup = new CSG.Vector(vertex.normal.x,vertex.normal.y,vertex.normal.z);
    }
  }
}

CSG.Polygon.prototype.setVerticesFromBackups = function(offset) {
  var vertex, vertexMap;
  for (var vi in this.vertices) {
    vertex = this.vertices[vi];
    vertex.pos.x = vertex.backup.x + offset.x;
    vertex.pos.y = vertex.backup.y + offset.y;
    vertex.pos.z = vertex.backup.z + offset.z;
    vertexMap = vertex.vertexMap;
    if (vertexMap) {
      for (var equivalentVertex of vertexMap) {
        equivalentVertex.pos.x = equivalentVertex.backup.x + offset.x;
        equivalentVertex.pos.y = equivalentVertex.backup.y + offset.y;
        equivalentVertex.pos.z = equivalentVertex.backup.z + offset.z;
      }
    }
  }
...
CSG.prototype.assignUuids = function() {
  ...
  for (var polygon of this.polygons) {
    ...
    polygon.csgObject = this; // hack for now
  }
}
...
// Winding number algo for in-plane polygons http://geomalgorithms.com/a03-_inclusion.html
// pointIsLeft(): tests if a point is Left|On|Right of an infinite line.
...
CSG.Polygon.prototype.pointIsLeft = function(p0, p1, p2) {
...
CSG.Polygon.prototype.pointInside = function(P) {
  var wn = 0;    // the  winding number counter
  var vertices = this.vertices;
  var vLen = vertices.length;
  var i1, i2;
  // loop through all edges of the polygon
  for (var i = 0; i < vLen; i++) {   // edge from V[i] to  V[i+1]
    i1 = i;
    i2 = (i + 1) % vLen;
    /* console.log('examining vertex index:',i, 'positions:', 
     *             vertices[i1].pos.x.toFixed(2), 
     *             vertices[i1].pos.y.toFixed(2), 
     *             vertices[i2].pos.x.toFixed(2), 
     *             vertices[i2].pos.y.toFixed(2));*/
    if (vertices[i1].pos.y <= P.y) {          // start y <= P.y
      // console.log('check for left');
      if (vertices[i2].pos.y > P.y) {      // an upward crossing
        //console.log('check left 2');
        if (this.pointIsLeft(vertices[i1].pos, vertices[i2].pos, P) > 0) { // P left of  edge
          //console.log('P left of edge');
          wn++;
        }
      }
    } else {                        // start y > P.y (no test needed)
      //console.log('check for right');
      if (vertices[i2].pos.y <= P.y) {    // a downward crossing
        //console.log('check right 2');
        if (this.pointIsLeft( vertices[i1].pos, vertices[i2].pos, P) < 0)  { // P right of  edge
          //console.log('P right of edge');
...
CSG.prototype.checkPointInPoly = function() {
  var P = new CSG.Vector(-0.5,0.25,0);
  for (var polygon of this.polygons) {
    if (polygon.plane.normal.z == -1) {
      console.log('Examining polygon:', polygon, 'normal:', polygon.plane.normal);
      console.log('point inside=', polygon.pointInside(P));
    }
  }
}
...
CSG.Vector.prototype.rotateVertexOnAxis = function(axisVector, angle) {
  var vertexThreeJs = new THREE.Vector3(this.x, this.y, this.z);
  vertexThreeJs.applyAxisAngle(axisVector, angle);
  this.x = vertexThreeJs.x;
  this.y = vertexThreeJs.y;
  this.z = vertexThreeJs.z;
}

// Rotating a point about an arbitrary axis: http://paulbourke.net/geometry/rotate/
// Python version at http://paulbourke.net/geometry/rotate/PointRotate.py

// This function assumes the axisVector is already a unit vector, and the angle is in radians...
// but we have to subtract the rotateTool position (rotation origin) from the vertices before rotating them, and then add it back after rotation.
CSG.prototype.rotateOnAxis = function(origin, axisVector, angle) {
  var newVertex, newNormal;
  var vertexThreeJs;
  //console.log('origin:', origin);
  for (var polygon of this.polygons) {
    for (var vertex of polygon.vertices) {
      newVertex = vertex.backup.clone().minus(origin);
      newVertex.rotateVertexOnAxis(axisVector,angle);
      newNormal = vertex.normalBackup.clone().minus(origin);
      newNormal.rotateVertexOnAxis(axisVector,angle);
      vertex.pos = newVertex.plus(origin);
      vertex.normal = newNormal.plus(origin);
    }
    polygon.plane = CSG.Plane.fromPoints(polygon.vertices[0].pos, polygon.vertices[1].pos, polygon.vertices[2].pos);
  }
  this.setBoundingBoxes();
}

CSG.prototype.createMesh = function(parent, material) {
  var cGeo = this.toMesh();
  parent.remove(this.mesh);
  this.mesh = new THREE.Mesh(cGeo, material);
  this.mesh.geometry.computeFaceNormals();
  parent.add(this.mesh);
}

CSG.prototype.setSelectMeshStatus = function(opts) {
//  if ((this.selectMesh.status & SELECT_STATUSES.PICKED) && (opts.action == 'remove')) {
//    debugger;
//  }
  if (opts.action == 'add') {
    this.selectMesh.status |= opts.status;
  } else {
    this.selectMesh.status &= ~opts.status;
  }

  this.selectMesh.material = SELECTMESH_MATERIALS[this.selectMesh.status];
  if (this.selectMesh.status == SELECT_STATUSES.HIDDEN) {
    this.selectMesh.position.x = -1e10;
  } else {
    this.selectMesh.position.x = 0;
  }
}

CSG.prototype.createSelectMesh = function(parent, status) {
  if (this.selectMesh) {
    parent.remove(this.selectMesh);
  }
  this.selectMesh = this.mesh.clone();
  this.selectMesh.status = status;
  this.selectMesh.scale.multiplyScalar(1.04);
  parent.add(this.selectMesh);

  this.setSelectMeshStatus({ action: 'add', status: SELECT_STATUSES.HIDDEN });
}

CSG.prototype.completeRotate = function(origin, rotationVector, angle, parent, material, pickStatus) {
  this.rotateOnAxis(origin, rotationVector, angle * DEG_TO_RAD);
  this.createMesh(parent, material);
  this.createSelectMesh(parent, pickStatus);
  firstRender = true;
}

CSG.prototype.animateRotate = function(origin, rotationVector, angle, parent, material, steps) {
  this.animation = { 
    type: 'rotate',
    origin: origin,
    parent: parent,
    ...
    steps: steps,
    stepCount : 0,
    subAngle: angle / steps
  }
}

CSG.prototype.animate = function(material, pickStatus) {
  var animation = this.animation;
  var stillRotating = true;
  if (animation.stepCount > animation.steps) {
    this.bsp = new CSG.Node(this.polygons);
    this.createSelectMesh(parent, pickStatus);
    this.animation = undefined;
    stillRotating = false;
  } else {
    this.rotateOnAxis(animation.origin, animation.rotationVector, (animation.subAngle * animation.stepCount) * DEG_TO_RAD);
    this.createMesh(animation.parent, animation.material);
    animation.stepCount++;
  }      
  firstRender = true;
