...
CSG.Polygon.prototype.pointInside = function(P) {
  ...
  for (var i = 0; i < vLen; i++) {   // edge from V[i] to  V[i+1]
    if (vertices[i].pos.y <= P.y) {          // start y <= P.y
      if (vertices[i+1].pos.y > P.y) {      // an upward crossing
        if (this.pointIsLeft(vertices[i].pos, vertices[i+1].pos, P) > 0) { // P left of  edge
          wn++;
        } else {                        // start y > P.y (no test needed)
          if (vertices[i+1].pos.y <= P.y) {    // a downward crossing
            if (isLeft( vertices[i].pos, vertices[i+1].pos, P) < 0)  { // P right of  edge
